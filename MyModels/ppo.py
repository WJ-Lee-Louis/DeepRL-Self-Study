    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                logratio = newlogprob - b_logprobs[mb_inds] # log pi(a|s) - log pi_old(a|s)
                ratio = logratio.exp() # pi(a|s) / pi_old(a|s): 여기서 log를 exp 취해주면서 policy ratio로 변환하는 모습
                ...
                pg_loss = torch.max(pg_loss1, pg_loss2).mean() # 실제 구현에서는 minmization으로 접근하기 때문에 max 처리
